...
\### 🤖 Phase 2 — AI-Assisted GUI (`employee\_gui\_final.py`)
...
em\_performance — for performance and experience tracking
...
python employee\_gui\_final.py





Run the original console version if you want(First prototype without interface):
...
em\_no	VARCHAR(20)	Primary key (manual Employee ID entered by user)

em\_name	VARCHAR(100)	Employee name

em\_dept	VARCHAR(100)	Department

em\_salary	FLOAT	Salary amount

em\_age	INT	Age of the employee



Table: em\_performance

Column	Type	Description

id	INT AUTO\_INCREMENT	Primary key

em\_no	VARCHAR(20)	Foreign key (references office.em\_no)
...
├── employee\_gui\_final.py     ← Tkinter GUI version (AI-assisted)
...
By maintaining both versions — the manual CLI (mainp.py) and the AI-assisted GUI (employee\_gui\_final.py) — this project shows both programming capability and modern AI fluency.
...
The GUI version of this project (employee\_gui\_final.py) was developed with the assistance of ChatGPT (OpenAI), based on the logic and database design from the original mainp.py.
